...
type: docs
weight: 20
url: /ru/jasperreports/installation/integration-with-jasperserver/
---
{{% alert color="primary" %}}
Чтобы интегрировать Aspose.CAD для JasperReports с JasperServer, необходимо выполнить несколько дополнительных шагов и обновить конфигурационные файлы JasperServer. Эта статья объясняет, как это сделать.
{{% /alert %}}
1. Добавьте новые свойства экспортера в файл конфигурации %INTALL_DIR%\apache-tomcat\webapps\jasperserver\WEB-INF\flows\viewReportBeans.xml.
{{< highlight xml >}}
    <!--JPG-->
    ...
    <bean id="jpgASExporterParameters" class="com.jaspersoft.jasperserver.war.action.ExporterConfigurationBean">
        <property name="descriptionKey" value="JPG - Экспорт изображения из Aspose.CAD"/>
    ...
    <bean id="bmpASExporterParameters" class="com.jaspersoft.jasperserver.war.action.ExporterConfigurationBean">
        <property name="descriptionKey" value="BMP - Экспорт изображения из Aspose.CAD"/>
    ...
    <bean id="gifASExporterParameters" class="com.jaspersoft.jasperserver.war.action.ExporterConfigurationBean">
        <property name="descriptionKey" value="GIF - Экспорт изображения из Aspose.CAD"/>
    ...
    <bean id="jpg2000ASExporterParameters" class="com.jaspersoft.jasperserver.war.action.ExporterConfigurationBean">
        <property name="descriptionKey" value="JPG2000 - Экспорт изображения из Aspose.CAD"/>
    ...
    <bean id="pdfASExporterParameters" class="com.jaspersoft.jasperserver.war.action.ExporterConfigurationBean">
        <property name="descriptionKey" value="PDF - Экспорт изображения из Aspose.CAD"/>
    ...
    <bean id="pngASExporterParameters" class="com.jaspersoft.jasperserver.war.action.ExporterConfigurationBean">
        <property name="descriptionKey" value="PNG - Экспорт изображения из Aspose.CAD"/>
    ...
    <bean id="psdASExporterParameters" class="com.jaspersoft.jasperserver.war.action.ExporterConfigurationBean">
        <property name="descriptionKey" value="PSD - Экспорт изображения из Aspose.CAD"/>
    ...
    <bean id="svgASExporterParameters" class="com.jaspersoft.jasperserver.war.action.ExporterConfigurationBean">
        <property name="descriptionKey" value="SVG - Экспорт изображения из Aspose.CAD"/>
    ...
    <bean id="tiffASExporterParameters" class="com.jaspersoft.jasperserver.war.action.ExporterConfigurationBean">
        <property name="descriptionKey" value="TIFF - Экспорт изображения из Aspose.CAD"/>
    ...
    <bean id="wmfASExporterParameters" class="com.jaspersoft.jasperserver.war.action.ExporterConfigurationBean">
        <property name="descriptionKey" value="WMF - Экспорт изображения из Aspose.CAD"/>
...
{{< /highlight >}}
    ...
    Чтобы изменить режим экспорта, используйте свойство с именем "exportMode" и установите значение в соответствии с желаемым режимом. 
Например:
{{< highlight xml >}}
    <property name="exportMode">
        <value type="com.aspose.cad.jasperreports.common.ExportMode">SinglePage</value>
...
{{< /highlight >}}
    Вы можете изменить значение свойства на Batch, SinglePage или Multipage, используя перечисление com.aspose.cad.jasperreports.common.ExportMode.

2. Найдите элемент <util:map id="exporterConfigMap">...</util:map> в файле %INTALL_DIR%\\apache-tomcat\webapps\jasperserver\WEB-INF\flows\viewReportBeans.xml и добавьте следующие строки:
{{< highlight xml >}}
    <util:map id="exporterConfigMap">
        <!-- закомментируйте/раскомментируйте любые строки ниже, если хотите исключить/включить соответствующих экспортеров 
        	 в список экспортеров просмотрщика
        	 Примечание: отдельная конфигурация для iPad 'exportersSupportedByiPad'
        	 -->
        ...
        -->
			...
			<!-- добавьте эти записи в exporterConfigMap -->
		...
		<!-- Aspose.CAD для JasperReports КОНЕЦ -->
    </util:map>
{{< /highlight >}}
3. Скопируйте aspose-cad-jasperreports-xx.xx.jar в %INTALL_DIR%\apache-tomcat\webapps\jasperserver\WEB-INF\lib.
4. Чтобы использовать функции экспорта, обновите %INTALL_DIR%\apache-tomcat\webapps\jasperserver\WEB-INF\applicationContext.xml следующим образом.
{{< highlight xml >}}
    <bean id="jpgExportParameters" class="com.aspose.cad.jasperreports.jpg.ASJpegExportParametersBean">
	<!--        Раскомментируйте и измените, чтобы применить лицензию. Проверьте путь к лицензии.
...
5. Перезагрузите сервер JasperReports и откройте любой отчет для просмотра. Если предыдущие шаги были выполнены правильно, вы увидите дополнительные варианты в списке форматов экспорта.
![todo:image_alt_text](/cad/_assets/jasper/ExportReportView.png)

Если вы не видите дополнительных форматов (значков или вариантов), пожалуйста, проверьте журналы в каталоге \apache-tomcat\logs.
